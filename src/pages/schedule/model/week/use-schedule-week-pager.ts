...
export type CurrentSlotIndex = 0 | 1 | 2 | 3 | 4;
...
export function useScheduleWeekPager({
  ...
  >({
    ...
    2: { date: currentDate.add(2, 'week'), logicalPage: 2, isReady: true },
    3: { date: currentDate.subtract(2, 'week'), logicalPage: -2, isReady: true },
    4: { date: currentDate.subtract(1, 'week'), logicalPage: -1, isReady: true },
  });
  ...
  // Synchronize ONLY on external date changes (e.g. calendar reset or external switcher)
  ...
  // Prefetch lookahead buffer of months for active 5-screen window in background
  useEffect(() => {
  ...
  const commitNavigation = useCallback(
    (direction: -1 | 1) => {
      ...
      const nextSlotIndex = ((urgentState.currentSlotIndex + direction + 5) %
        5) as CurrentSlotIndex;
      const nextLogicalPage = urgentState.logicalPageIndex + direction;

      // Far slot ahead (2 steps ahead in swipe direction) to recycle
      const farSlotIndex = ((nextSlotIndex + direction * 2 + 5) %
        5) as CurrentSlotIndex;
      const farLogicalPage = nextLogicalPage + direction * 2;
      const farDate = nextDate.add(direction * 2, 'week');
  ...
  const slots: PhysicalSlot[] = useMemo(() => {
    const indices: CurrentSlotIndex[] = [0, 1, 2, 3, 4];
